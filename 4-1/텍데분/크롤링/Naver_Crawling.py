...
# Keyword 및 Parameter 설정
news_list = ['국민의힘','날씨','민주당','의대','전쟁','정부','금리','부동산','주식','구글','네이버','삼성','애플']
start_list = [1, 101, 201, 301]
n_display = 100
...
for start in start_list:
    for keyword in news_list:
        ...
        for item in search_results['items']:
            ...
            if 'n.news.naver.com' in link: # "진짜" 네이버 뉴스 link만 사용
                all_results[i] = dict()
        ...
        folder_path = f"I:/내 드라이브/4-1/텍데분/크롤링/크롤링폴더/{keyword}"
        now = datetime.datetime.now() 
        file_path = os.path.join(folder_path, f'{keyword}_{now.strftime("%Y%m%d")}_{start}.csv')
